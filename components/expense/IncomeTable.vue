    ...
    <v-card class="bg-grey-darken-3 h-100">
      ...
      <v-card-text>
        <v-row>
          ...
          <v-col cols="1">
            <v-dialog v-model="dialogState" width="500">
              ...
              <v-card>
                <v-toolbar color="#7E57C2" title="เพิ่มรายรับ"></v-toolbar>
        ...
        <v-data-table :loading="tableLoaded" :search="searchText" :headers="tableHeaders" :items="tableData"
          no-data-text="ไม่พบข้อมูลรายรับ" loading-text="โหลดข้อมูลรายรับ">
...
const deleteExpense = async (expenseId: number) => {
  const { isConfirmed } = await Swal.fire({
    icon: 'question',
    html: '<p class="text-white">ยืนยันการลบข้อมูลรายรับใช่หรือไม่</p>',
    background: '#424242',
    iconColor: '#7E57C2',
